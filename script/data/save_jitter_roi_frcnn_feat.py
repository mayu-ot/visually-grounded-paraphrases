...
def save_jitter_frcnn_feat(split, device):
    ...
    data_itr = SerialIterator(
        data, batch_size=100, repeat=False, shuffle=False)
    ...
    np.save('data/region_feat/jitter_roi-frcnn_asp0.66_off0.4/%s' % split,
            feat)
    np.save('data/region_feat/jitter_roi-frcnn_asp0.66_off0.4/rois_%s' % split,
            roi_arr)
