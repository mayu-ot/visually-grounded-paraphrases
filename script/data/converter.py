...
def save_phr2phr_score(in_file, out_file):
    ...
    tmpl_item_2 = parse.compile("{:d}/{} : {} # {:d}/{} : {} # {:g} {:g} {:g} {:g}")
    ...
    with open(in_file, 'r') as f:
        for line in f:
            ...
            if line[0] == 'i':
                ...
                if line.split('# ')[-1] == '0':
                    reg1, cat1, phr1, reg2, cat2, phr2 = tmpl_item_1.parse(line)
                    s = 0
                else:
                    reg1, cat1, phr1, reg2, cat2, phr2, s1, s2, s3, s4 = tmpl_item_2.parse(line)
                    s = s1 * s3
...
def convert_phr2phr_score_file(phrase_pair_file, phrase2phrase_score_file, output_file):
    ...
    for _, row in phrase_pair.iterrows():
        ...
        res = score_df[(score_df.image == row['image']) & (score_df.phrase1 == org_phrase1) & (score_df.phrase2 == org_phrase2)]
        if len(res) == 0:
    ...
    with open(txt_file, 'r') as f:
        for i, line in enumerate(f):
            ...
            for item in re.findall(r'\[.*?\]', line):
                phr_id, category, phrase = parse.parse('[/EN#{:d}/{} {}]', item)
                category = category.split('/')[0]
                if category != 'notvisual':
                    entities.append({'cap_i': i, 'id':phr_id, 'category': category, 'org_phrase': phrase})
            ...
            if line[:5] == 'image':
                ...
                for s1, s2 in combinations(sentences, 2):
                    for ent1, ent2 in product(s1, s2):
                        ...
                        items.append([im_id, ent1['org_phrase']+'/'+ent1['prepro_phrase'], ent2['org_phrase']+'/'+ent2['prepro_phrase'], y_true])
...
def remove_trivial_match(in_file, out_file):
    ...
    for _, row in df.iterrows():
        ...
        item.append([row['image'], phr1, phr2, org_phr1, org_phr2, row['ytrue']])

    df = pd.DataFrame(item, columns=['image', 'phrase1', 'phrase2', 'original_phrase1', 'original_phrase2','ytrue'])
    df.to_csv(out_file)

def save_phrase2phrase_score(phrase_pair_file, uni_phrase_file, feat_file, out_file):
...
def parse_entity_region_score(in_file, phrase_pair_file, out_name):
    ...
    with open(in_file, 'r') as f:
        ...
        for line_i, line in enumerate(f):
            ...
            if line[:3] == 'img':
                im_id, s_id, ent_id, group, phrase = parse.parse('img_{:d}:sen_{:d}:entity_{:d}/{}:{} ', line)
                ...
                if any(sub_df.original_phrase1 == phrase):
                    cvrt_phrase = sub_df[sub_df.original_phrase1 == phrase].phrase1.values[0]
                elif any(sub_df.original_phrase2 == phrase):
                    cvrt_phrase = sub_df[sub_df.original_phrase2 == phrase].phrase2.values[0]
                else:
                    cvrt_phrase = 'None'
                BB = []
            elif line[:3] == 'box':
                box_id, score, xmin, ymin, xmax, ymax = parse.parse('box: {:d}, score: {:f}, coordinate: {:f}, {:f}, {:f}, {:f}, ',line)
                items.append([im_id, s_id, ent_id, group, phrase, cvrt_phrase, box_id, xmin, ymin, xmax, ymax, score])
            else:
                pass
            
            bar.update(line_i)

    df = pd.DataFrame(items, columns=['image', 'sentence', 'entity', 'group', 'original_phrase', 'phrase','bbox', 'xmin', 'ymin', 'xmax', 'ymax', 'score'])
    df.to_csv(out_name)
...
def save_phrase_graph(in_file, out_dir):
    ...
    for im in images:
        ...
        G = nx.Graph()
        G.add_weighted_edges_from([(row['phrase1'], row['phrase2'], row['score']) for _, row in sub_df.iterrows()])

        p2i = {p:i for i, p in enumerate(G.nodes())}
        edges = [([p2i[p1], p2i[p2]]) for p1, p2 in G.edges()]
        weights = [w for _, _, w in G.edges(data='weight')]

        graph_data = {'edges': edges, 'weights': weights, 'phrases':[p for p in G.nodes()]}
...
def save_phrase2region_table(in_file, graph_dir, out_dir):
    ...
    for im in images:
        ...
        for i, p in enumerate(phrases):
            ...
            for _, row in sub_df[(sub_df.phrase == p) & (sub_df.sentence == sent_id)].iterrows():
...
def write_gt_cluster_label_file(in_file, out_file):
    ...
    for im_id in pd.unique(df.image.values):
        ...
        for s in flatten(sent):
            ...
            for phr in phr_list:
                if sub_df.phrase1.isin([phr]).any() or sub_df.phrase2.isin([phr]).any():
...
def main():
    ...
    save_phrase_pairs('data/pl-clc_cca/entity/%s/phraseWords'%split,
    ...
    phrase_score_file = out_base+'phrase_score_%s_%s.csv'%(feat_type, split)

    save_phrase2phrase_score(phrase_pair_filtered,
    ...
    parse_entity_region_score('data/pl-clc_cca/entity_region_scores.%sSplit.txt'%split,
