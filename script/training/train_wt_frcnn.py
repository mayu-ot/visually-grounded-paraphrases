...
class iParaphraseNet(chainer.Chain):
    def __init__(self):
        ...
        with self.init_scope():
            self.base_net = FasterRCNNExtractor(n_fg_class=20, pretrained_model='voc07')
...
def train(
    ...
    updater = training.StandardUpdater(train_iter, opt, converter=my_converter, device=device)
    ...
    trainer.extend(extensions.Evaluator(val_iter, model, converter=my_converter, device=device),
...
def main():
    ...
    parser.add_argument('--b_size', '-b', type=int, default=20,
