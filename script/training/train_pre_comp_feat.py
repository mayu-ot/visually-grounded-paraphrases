...
class PhraseProjectionNet(chainer.Chain):
    ...
    def setup_layers(self):
        ...
        self.fuse_2 = L.Linear(None, 300, initialW=initializers.HeNormal(), nobias=True)
...
class ImageProjectionNet(chainer.Chain):
    ...
    def setup_layers(self):
        h_size = 1000
        # fusenet for phrase and region features
        self.fuse_r1 = L.Linear(None, h_size, initialW=initializers.HeNormal(), nobias=True)
        self.bn_1 = L.BatchNormalization(h_size)
        self.fuse_r2 = L.Linear(None, h_size, initialW=initializers.HeNormal(), nobias=True)
        self.bn_2 = L.BatchNormalization(h_size)
        self.fuse_2 = L.Linear(None, 300, initialW=initializers.HeNormal(), nobias=True)
        self.bn_3 = L.BatchNormalization(300)
...
class ClassifierNet(chainer.Chain):
    def __init__(self, dr_ratio=.4):
        ...
        with self.init_scope():
            self.mlp_1 = L.Linear(None, 128, initialW=initializers.HeNormal(), nobias=True)
...
def train(
    ...
    if model_type in ['vis+lng+gtroi', 'vis+gtroi']:
        ...
        train = PLCLCDataset('train', san_check=san_check) # train on plclc bbox
        val = PLCLCDataset('val', san_check=san_check) # validate on plclc bbox
    elif model_type in ['vis+lng+ddpnroi', 'vis+ddpnroi']:
        train = DDPNDataset('train', san_check=san_check) # train on plclc bbox
        val = DDPNDataset('val', san_check=san_check) # validate on plclc bbox
    else:
        train = GTJitterDataset('train', san_check=san_check)
        val = PLCLCDataset('val', san_check=san_check) # validate on plclc bbox
    ...
    if model_type in ['vis+lng', 'vis+lng+gtroi', 'vis+lng+plclcroi', 'vis+lng+ddpnroi']:
    ...
    updater = training.StandardUpdater(train_iter, opt, converter=cvrt_pre_comp_feat, device=device)
    ...
    trainer.extend(extensions.Evaluator(val_iter, model, converter=cvrt_pre_comp_feat, device=device),
    ...
    if not san_check:
        ...
        best_val_trigger = training.triggers.MaxValueTrigger('validation/main/classifier/f1', trigger=val_interval)
    ...
    trainer.extend(extensions.PrintReport(['epoch', 'iteration','main/classifier/loss', 'main/validation/classifier/loss']), trigger=log_interval)
...
def get_prediction(model_dir, split, device=None):
    ...
    test_iter = SerialIterator(test, batch_size=1000, repeat=False, shuffle=False)
