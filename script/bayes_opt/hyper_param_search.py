...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_iter', '-i', type=int, default=None,
    ...
    bounds = [
        {'name': 'lr', 'type': 'continuous', 'domain': (-5, -1), 'dimensionality': 1},
        {'name': 'dr_ratio', 'type': 'continuous','domain': (0., .9), 'dimensionality': 1},
        {'name': 'w_decay', 'type': 'continuous','domain': (-7, -3), 'dimensionality': 1},
    ]
    ...
    report_file = out_dir + 'bo_report_%s.txt' % ("{0:%Y-%m-%d %H:%M:%S}".format(now))
    evaluation_file = out_dir + 'bo_eval_%s.txt' % ("{0:%Y-%m-%d %H:%M:%S}".format(now))
    prob.run_optimization(args.max_iter, report_file=report_file, evaluations_file=evaluation_file)