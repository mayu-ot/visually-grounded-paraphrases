...
def jitter_bbox(bbox, im_sizes, aspect_band=4 / 5, offset_band=0.1):
    ...
    w_ratio = np.asarray(
        [random.uniform(aspect_band, 1 / aspect_band) for _ in range(n)])
    h_ratio = np.asarray(
        [random.uniform(aspect_band, 1 / aspect_band) for _ in range(n)])
    w *= w_ratio
    h *= h_ratio

    x_offset = w * np.asarray(
        [random.uniform(-offset_band, offset_band) for _ in range(w.size)])
    y_offset = h * np.asarray(
        [random.uniform(-offset_band, offset_band) for _ in range(w.size)])
