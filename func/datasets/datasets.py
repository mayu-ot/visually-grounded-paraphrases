...
def get_agg_roi_df(split):
    gtroi_df = pd.read_csv('data/gt_roi_cord_%s.csv' % split, index_col=0)
...
def prepare_vis_feat_indices(bbox_df, pair_df):
    ...
    for _, row in d_df.iterrows():
        ...
        if img != cur_img:
            ...
            cur_img = img
            indices.setdefault(img, {})

        dist = list(map(lambda x: edit_distance(phrase.lower(), x), rows.phrase))
        row = rows.iloc[np.argmin(dist)]
        index = row.name
        indices[img][phrase] = int(index)

    return indices

class DDPNBBoxDataset(chainer.dataset.DatasetMixin):
    def __init__(self, split):
        df = pd.read_csv('data/ddpn/fix_split_%s.csv' % split)
        sub_df = pd.DataFrame(df.iloc[:, 4:8].values, columns=['xmin', 'ymin', 'xmax', 'ymax'])
...
class iParaphraseDataset(chainer.dataset.DatasetMixin):
    def __init__(self, split, san_check=False):
        ...
        self.gtroi_data = pd.read_csv('data/gt_roi_cord_%s.csv' % split, index_col=0)
...
class GTJitterDataset(PreCompFeatDataset):
    ...
    def setup(self, split):
        ...
        self.vis_feat = np.load('data/region_feat/jitter_roi-frcnn_asp0.66_off0.4/%s.npy' % split)
        if split == 'test':
            self.jbbox = np.load('data/region_feat/jitter_roi-frcnn_asp0.66_off0.4/rois_%s.npy' % split)
...
class DDPNDataset(PreCompFeatDataset):
    ...
    def setup(self, split):
        ...
        self.vis_feat = np.load('data/region_feat/ddpn_roi-frcnn/%s.npy' % split)
        
        indices_file = 'data/ddpn/vis_indices_%s.json' % split
        if os.path.exists(indices_file):
            vis_indices = json.load(open(indices_file))
        else:
            vis_indices = prepare_vis_feat_indices(self.bbox_df, self.pair_df)
    ...
    def prepare_vis_feat_indices(self, split):
        if os.path.exists('data/ddpn/vis_indices_%s.json' % split):
            return json.load(open('data/ddpn/vis_indices_%s.json' % split))
            
        phr1 = self.pair_data[['image', 'original_phrase1']].drop_duplicates()
        phr1 = phr1.rename(index=str, columns={'original_phrase1': 'phrase'})
        phr2 = self.pair_data[['image', 'original_phrase2']].drop_duplicates()
        phr2 = phr2.rename(index=str, columns={'original_phrase2': 'phrase'})
        d_df = pd.concat([phr1, phr2]).drop_duplicates()
        ...
        for _, row in d_df.iterrows():
            img, phrase = row
            if img != cur_img:
                rows = self.bbox_df[self.bbox_df.image == img]
                cur_img = img
                indices.setdefault(img, {})
        ...
        json.dump(indices, open('data/ddpn/vis_indices_%s.json' % split, 'w'))
...
class PLCLCDataset(DDPNDataset):
    def setup(self, split):
        ...
        indices_file = 'data/pl-clc/vis_indices_%s.json' % split
        if os.path.exists(indices_file):
            vis_indices = json.load(open(indices_file))
        else:
